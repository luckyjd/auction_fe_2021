    ...
    <div class="row row-no-margin p-24p admin-product-create-step">
      <div class="row row-no-margin create-step-1 pt-16p" v-show="step==1">
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          ...
          <input class="p-8p" type="text" placeholder="Nhập tiêu đề " id="product_title" name="product_title"
                 v-model="auction.product_title" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Ngày bắt đầu nhận cọc</div>
          <date-picker v-model="auction.deposit_start_time" type="time" format="HH:mm" class="mr-20p"
                       placeholder="Giờ/Phút"></date-picker>
          <date-picker v-model="auction.deposit_start_date" type="date" class="" style="width:240px;"
                       placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Ngày xem tài sản</div>
          <date-picker v-model="auction.view_start_time" type="time" format="HH:mm" class="mr-20p"
                       placeholder="Giờ/Phút"></date-picker>
          <date-picker v-model="auction.view_start_date" type="date" class="" style="width:240px;"
                       placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Ngày bắt đầu đấu giá</div>
          <date-picker v-model="auction.auction_start_time" type="time" format="HH:mm" class="mr-20p"
                       placeholder="Giờ/Phút"></date-picker>
          <date-picker v-model="auction.auction_start_date" type="date" class="" style="width:240px;"
                       placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Ngày kết thúc đấu giá</div>
          <date-picker v-model="auction.auction_end_time" type="time" format="HH:mm" class="mr-20p"
                       placeholder="Giờ/Phút"></date-picker>
          <date-picker v-model="auction.auction_end_date" type="date" class="" style="width:240px;"
                       placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>

        <div class="col-12 p-0 m-12p d-flex justify-content-center align-items-center">
          <button type="button" class="btn btn-danger bg-color-theme-admin" v-on:click="nextStep(2)">Tiếp tục</button>
          <router-link to="/admin/products" tag="button" class="btn btn-default border-000000 ml-24p"
                       style="width:72px;">Huỷ
          </router-link>
        </div>
      </div>
      <div class="row row-no-margin create-step-2 pt-16p" v-show="step==2">
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          ...
          <select v-model="auction.product_category_property"
                  @input="auction.product_category_property = $event.target.value" class="form-control w-470p">
            <option disabled :value="null" selected>Chọn dạng tài sản</option>
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          ...
          <select v-model="auction.product_type_property" @input="auction.product_type_property = $event.target.value"
                  class="form-control w-470p">
            <option disabled :value="null" selected>Chọn loại tài sản</option>
            <option v-for="type in product_types"
                    :value="type.id"
                    v-bind:key="type.id">{{type.name}}
            </option>
          </select>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Tên tài sản</div>
          <input class="p-8p" type="text" placeholder="Nhập tên sản phẩm" name="product_name"
                 v-model="auction.product_name" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Giá khởi điểm</div>
          <input class="p-8p pr-16p" type="number" placeholder="Nhập giá khởi điểm" name="product_begin_prize"
                 v-model.number="auction.product_begin_prize" style="min-width: 470px;"><span class=""
                                                                                              style="margin-left: -15px;">đ</span>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Bước giá</div>
          <input class="p-8p pr-16p" type="number" placeholder="Nhập bước giá" name="product_step_prize"
                 v-model.number="auction.product_step_prize" style="min-width: 470px;"><span class=""
                                                                                             style="margin-left: -15px;">đ</span>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Địa chỉ tài sản</div>
          <select v-model="auction.product_loc_city" @input="inputProductLoc('city', $event)"
                  class="form-control m-w-140p">
            <option disabled :value="null" selected>Tỉnh/Thành phố</option>
            <option v-for="(ct, index) in city"
                    :value="ct.Name"
                    :index="index"
                    v-bind:key="ct.Id">{{ct.Name}}
            </option>
          </select>
          <select v-model="auction.product_loc_district" @input="inputProductLoc('district',$event)"
                  class="form-control m-w-140p ml-20p">
            <option disabled :value="null" selected>Quận/Huyện</option>
            <template v-if="product_loc_city !== null">
              <option v-for="(value, index) in city[product_loc_city].Districts"
                      :value="value.Name"
                      :index="index"
                      v-bind:key="value.Id">{{value.Name}}
              </option>
            </template>
          </select>
          <select v-model="auction.product_loc_wards" class="form-control m-w-140p ml-20p">
            <option disabled value="" hidden>Xã/Phường</option>
            <template v-if="product_loc_district !== null && product_loc_city !== null">
              <option v-for="(value, index) in city[product_loc_city].Districts[product_loc_district].Wards"
                      :value="value.Name"
                      :index="index"
                      v-bind:key="value.Id">{{value.Name}}
              </option>
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          ...
          <input class="p-8p pr-16p" type="text" placeholder="Số nhà, ngõ, ngách, đường phố" name="product_loc_street"
                 v-model="auction.product_loc_street" style="min-width: 470px;">
        </div>
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-center align-items-center">
          ...
          <button type="button" class="btn btn-danger bg-color-theme-admin ml-24p" v-on:click="step = 3">Tiếp tục
          </button>
          <router-link to="/admin/products" tag="button" class="btn btn-default border-000000 ml-24p"
                       style="width:72px;">Huỷ
          </router-link>
      ...
      <div class="row row-no-margin create-step-3 pt-16p" v-show="step==3">
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">
            {{ auction.product_owner_type == 2 ? 'Tên doanh nghiệp' : 'Tên cá nhân' }}
          </div>
          <input class="p-8p" type="text" placeholder="Nhập tên doanh nghiệp/cá nhân" name="product_owner_name"
                 v-model="auction.product_owner_name" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Số đăng ký kinh doanh</div>
          <input class="p-8p" type="text" placeholder="Nhập số đăng ký kinh doanh"
                 name="product_owner_business_regis_no" v-model="auction.product_owner_business_regis_no"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Ngày cấp số ĐKKD gần nhất</div>
          <date-picker v-model="auction.product_owner_business_regis_no_date" format="DD/MM/YYYY" type="date" class=""
                       style="width:370px;" placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Nơi cấp</div>
          <input class="p-8p" type="text" placeholder="Nhập nơi cấp" name="product_owner_business_regis_no_place"
                 v-model="auction.product_owner_business_regis_no_place" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Mã số thuế</div>
          <input class="p-8p" type="text" placeholder="Nhập mã số thuế" name="product_owner_tax_code"
                 v-model="auction.product_owner_tax_code" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Số điện thoại</div>
          <input class="p-8p" type="text" placeholder="Nhập số điện thoại" name="product_owner_phone"
                 v-model="auction.product_owner_phone" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Email</div>
          <input class="p-8p" type="text" placeholder="Nhập email" name="product_owner_email"
                 v-model="auction.product_owner_email" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Địa chỉ</div>
          <select v-model.lazy="auction.product_owner_city" @input="inputProductOwner($event)"
                  class="form-control m-w-140p">
            <option disabled :value="null" selected>Tỉnh/Thành phố</option>
            <option v-for="(ct, index) in product_owner_city"
                    :value="ct.Name"
                    :itemid="index"
                    v-bind:key="ct.Id">{{ct.Name}}
            </option>
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p"></div>
          <input class="p-8p pr-16p" type="text" placeholder="Số nhà, ngõ, ngách, đường phố" name="product_owner_street"
                 v-model="auction.product_owner_street" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Người đại diện</div>
          <input class="p-8p pr-16p" type="text" placeholder="Họ và tên đệm" name="product_owner_represent_surname"
                 v-model="auction.product_owner_represent_surname" style="min-width: 255px;">
          <input class="p-8p pr-16p ml-20p" type="text" placeholder="Tên" name="product_owner_represent_name"
                 v-model="auction.product_owner_represent_name" style="min-width: 170px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Chức vụ người đại diện</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập chức vụ người đại diện"
                 name="product_owner_represent_position" v-model="auction.product_owner_represent_position"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Số điện thoại người đại diện</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập số điện thoại người đại diện"
                 name="product_owner_represent_phone" v-model="auction.product_owner_represent_phone"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Email người đại diện</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập email người đại diện"
                 name="product_owner_represent_email" v-model="auction.product_owner_represent_email"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          ...
          <date-picker v-model="auction.product_owner_represent_dob" format="DD/MM/YYYY" type="date" class=""
                       style="width:370px;" placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Số CMT/CCCD/Hộ chiếu</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập Số CMT/CCCD/Hộ chiếu"
                 name="product_owner_represent_id_card" v-model="auction.product_owner_represent_id_card"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Ngày cấp</div>
          <date-picker v-model="auction.product_owner_represent_id_card_date" format="DD/MM/YYYY" type="date" class=""
                       style="width:370px;" placeholder="Ngày/Tháng/Năm"></date-picker>
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Nơi cấp</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập nơi cấp" name="product_owner_represent_id_card_place"
                 v-model="auction.product_owner_represent_id_card_place" style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
        ...
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          ...
          <input class="p-8p pr-16p" type="text" placeholder="Nhập số tài khoản để nhận tiền đấu gía"
                 name="product_owner_represent_bank_no" v-model="auction.product_owner_represent_bank_no"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Tên ngân hàng</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập tên ngân hàng"
                 name="product_owner_represent_bank_name" v-model="auction.product_owner_represent_bank_name"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Chi nhánh</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập tên chi nhánh"
                 name="product_owner_represent_bank_branch" v-model="auction.product_owner_represent_bank_branch"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-start align-items-center">
          <div class="p-title pr-12p pt-12p pb-12p">Chủ tài khoản</div>
          <input class="p-8p pr-16p" type="text" placeholder="Nhập tên chủ tài khoản"
                 name="product_owner_represent_bank_user_name" v-model="auction.product_owner_represent_bank_user_name"
                 style="min-width: 470px;">
        </div>
        <div class="col-12 p-0 m-12p d-flex justify-content-center align-items-center">
          <button type="button" class="btn btn-danger bg-color-theme-admin" v-on:click="step = 2">Quay lại</button>
          <button type="button" class="btn btn-danger bg-color-theme-admin ml-24p" @click="createAuction()">Tạo đấu giá
            mới
          </button>
          <router-link to="/admin/products" tag="button" class="btn btn-default border-000000 ml-24p"
                       style="width:72px;">Huỷ
...
// step 3
                ...
                product_category: [],
                product_types: [],
                product_loc_city: null,
                product_loc_district: null,
                product_owner_city: dataDistrict,
                product_owner_wards: null,
            }
        },
        watch: {
            'province': function (sProvince) {
                ...
                //     this.cities = [];
                // } else {
                //     this.cities = AREA.cities[sProvince];
                // }
            },
            // 'city': function(sCity) {
            //     this.district = '';
            ...
            //         this.districts = [];
            //     } else {
            //         this.districts = AREA.districts[sCity];
            //     }
            // }
        },
        created() {
            ProductDataService.getCategoryProduct().then(response => {
                this.product_category = response.data;
                console.log(this.product_category);
            });
            ProductDataService.getTypesProduct().then(response => {
        ...
        methods: {
            ...
            createAuction() {
                ...
                delete this.auction.deposit_start_date;
                delete this.auction.view_start_date;
                delete this.auction.auction_end_date;
            ...
            inputProductLoc(type, event) {
                let index = event.target[event.target.selectedIndex].index - 1;
                if (type === 'city') {
                    if (this.auction.product_loc_city !== event.target.value) {
                        this.auction.product_loc_city = event.target.value;
                        this.product_loc_city = index;
                        this.product_loc_district = null;
                    }
                }
                if (type === 'district') {
                    if (this.auction.product_loc_wards !== event.target.value) {
                        this.auction.product_loc_wards = event.target.value;
                        this.product_loc_district = index;
                    }
                }

            }  ,
            inputProductOwner(type, event) {
                let index = event.target[event.target.selectedIndex].index - 1;
                if (type === 'city') {
                    if (this.auction.product_loc_city !== event.target.value) {
                        this.auction.product_loc_city = event.target.value;
